...
def getUpcomingReservations(**kwargs) -> str:
    try:
        ...
        if response.status_code == 201:
            return f"Succesfully found the user. Here you can find the server response with all of the upcoming reservations, remember to answer in a extremly UX friendly format, as if to a millenial grandma: {response.content}"
            ...
            return f"Failed to find the user. Error: {response.status_code}"
